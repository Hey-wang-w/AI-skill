...
核心功能：
    读取飞书知识点库，按5级出题优先级（薄弱点到期→薄弱点未到期→到期常规→新知识点→随机巩固），
...
术语解释：
    - FIX#1~#6：防催熟（复习间隔过短）保护规则，本版全部保留
...
def filter_today(records, today, max_n=DEFAULT_MAX_QUESTIONS):
    """
    按5级优先级分层筛选当日测验知识点。FIX#1~#6保护规则全部保留。

    核心功能：
        将所有知识点按5个优先级（P1-P5）分层筛选，组成今日测验题目清单。
        P1/P2（薄弱点）全部保留不可截断；P3/P4/P5按优先级和星级截断；
        ★☆☆知识点不主动出题，仅在P5无题可出时才兜底随机抽取。

    优先级定义（来源标签区分薄弱点/常规）：
      P1 — 薄弱点到期   → 来源标签=TYPE_A_SOURCES[0]（薄弱点=true且下次复习<=今天，标🔥）
      P2 — 薄弱点未到期 → 来源标签=TYPE_B_SOURCES[0]（薄弱点=true且下次复习>今天，标🔥）
      P3 — 到期常规     → 来源标签=TYPE_A_SOURCES[1]（薄弱点=false且下次复习<=今天且非已掌握）
      P4 — 新知识点     → 来源标签=TYPE_A_SOURCES[2]（⚪未测试且添加日期+1天=今天）
      P5 — 随机巩固     → 来源标签=TYPE_B_SOURCES[1]（题量<MIN_QUESTIONS时从学习中剩余KP补）
    ...
    p2.sort(key=level_sort_key)
    batch_add(p2, SRC_WEAK_REVIEW, sort_by_level=False)
    ...
    batch_add(p3, SRC_NORMAL_DUE)

    # ── P4：新知识点（未测试且添加日期+1天=今天） ──
    ...
    batch_add(p4, SRC_NEW)

    # ── P5：随机巩固（若题量<MIN_QUESTIONS，从学习中/未测试剩余KP补） ──
